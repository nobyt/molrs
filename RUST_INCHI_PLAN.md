...
> **v1 実装状況 (2026-08-03 更新)**: I0–I15 実装済み。実測 (コーパス 7,453 分子、
> RDKit オラクル):
...
>   残る文字列不一致の主因は (a) 縮環系の橋頭ヘテロ原子 (例: インドリジン
>   型の 3 配位 N、二重結合を一切持たない) に隣接する環外ドナー (例:
>   縮環フェノール性 OH) が、無関係な環内ヘテロ原子まで誤って橋渡しする
>   ことがある残存ケース (環外ドナーの孤立供与体ブリッジを芳香環メンバー
>   限定にすると、今度は正当な環外アミノ基のケースが壊れるため、単純な
>   追加規則では未解決— おそらく I14 で見送った容量付きバランスドネット
>   ワークの完全実装が必要)、(b) 一部の縮合環 c 層直列化、(c) 立体絡みの
>   ケース。詳細はコーパス実測で継続監視 (`tests/inchi_gate.rs` 参照)。
