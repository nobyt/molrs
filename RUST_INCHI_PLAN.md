...
> **v1 実装状況 (2026-07-26 更新)**: I0–I12 実装済み。実測 (コーパス 7,453 分子、
> RDKit オラクル):
...
>   なし): **84.50%** が RDKit と完全一致 (I4 時点は 74.17%、詳細な推移は下記)
...
>   残る文字列不一致の主因は**芳香環をまたぐ多中心互変異性**
>   (アミノピリミジン・ウラシル系ジオン・プリン様縮合環など — 単一中心の
>   星型検出では届かず、環全体を通る交互結合パスの探索が必要) と
>   一部の縮合環 c 層直列化 — 逐次拡張 (I13 候補)。
