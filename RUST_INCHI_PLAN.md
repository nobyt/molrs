...
>   ラクタム/一級スルホンアミドを新たに正しく処理。フル InChI 一致
>   74.19% → **78.04%** (5714/7322)。
> - v1 の非対応 (→ `Unsupported`): 電荷 (q/p 層)・多成分・同位体 (i)・
>   有機金属切断。残る文字列不一致の主因は多中心の環互変異性 (イミダゾール/
>   トリアゾール/ピリジノン等) と一部の縮合環 c 層直列化・複雑立体 — 逐次拡張。
