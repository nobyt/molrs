...
mod tests {
    ...
    /// I53: 硫酸ナトリウムの実 InChI は `2Na.H2O4S` (カチオン先頭)。旧規則
    /// (炭素数降順→重原子数降順) では `H2O4S.2Na` になり説明できなかったが、
    /// `ichimake.c::CompareHillFormulasNoH` 準拠のトークン単位比較
    /// (H を除いた式を要素ごとに比較、Na < O のため Na が先) で正しく
    /// 再現できる。旧テスト名は「既知の残差」だったが、これはもう残差では
    /// ない。
    #[test]
    fn monoatomic_cation_before_polyatomic_anion() {
        assert_eq!(
