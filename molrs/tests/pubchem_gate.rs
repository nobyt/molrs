...
fn pubchem_full_inchi() {
    ...
    //               `mt`7→4)。
    //            → I74 99.64% (`is_undefined_tetra` の N 判定に、中性 3 員環
    //               (アジリジン型) N を追加。charge>0 の N しか対象外にして
    //               いなかったが、`inchi-1` で実測すると N-アルキルアジリジン
    //               (孤立電子対中心・重原子3・H0) は中性でも `?` が付き、
    //               鎖状第三級アミンや 4 員環 (アゼチジン) 以上の N には付かな
    //               い — 環の 2 個の重原子隣接どうしが直接結合 (= 3 員環) して
    //               いることを条件に追加。`t`21→17)。
    //            → I75 99.65% (`is_protonatable` の O/S/Se/Te 分岐で、脱プロ
    //               トン化中心自身が O/S への実二重結合を持つ場合 (`C[S-]
    //               (=S)=S` のような多硫化物型・`C[S-](=O)` のようなスルフィ
