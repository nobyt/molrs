...
fn full_inchi_matches_rdkit_where_produced() {
    ...
    // I14 到達点 = 89.33% (v1 適用範囲 = 中性・単一成分・立体/同位体なし)。
    // IUPAC 公式 InChI ソース (ichitaut.c/ichi_bns.c) を参照し、可動 H
    // 検出をブロッサム法ベースに刷新: (1) 受容体判定は特定の結合が二重かで
    // はなく原子自身の価数余裕 (集約的に二重結合を 1 本持つか) で行う、
    // (2) 縮環の共有原子は環ごとに頂点分割し、ある環の辺で到達した探索が
    // 別環の辺へ無条件に「乗り換える」のを防ぐ (縮環の共有原子が両環の
    // ヘテロ原子と直接隣接する場合のみ種形成時に正しく拾われる)。
    // 残る不一致の主因は一部の縮合環 c 層直列化と立体絡みのケース。
    // 閾値は退行検知用。
    assert!(acc >= 0.892, "full InChI accuracy {acc:.4} < 0.892");
}
...
fn to_inchi_key_matches_rdkit_where_produced() {
    ...
    assert!(acc >= 0.892, "to_inchi_key accuracy {acc:.4} < 0.892");
}
