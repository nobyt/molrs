...
fn full_inchi_matches_rdkit_where_produced() {
    ...
    // I19 §3.4 (2026-08-05): I16 以来の既知の残差 (縮環系の橋頭ヘテロ原子
    //     に隣接する環外ドナーの誤った橋渡し) を解消。橋頭ヘテロ原子
    //     (価数スラック 0・次数 3 以上、例: インドリジン型の 3 配位 N) を
    //     含む環は、その環自体とその融合相手の環も含めて、複数原子をまたぐ
    //     ブリッジ探索 (辺・孤立供与体起点) の対象から完全に除外する
    //     (poisoned_ring_atoms)。ただし単一中心の局所パターン
    //     (seed_groups、実在する二重結合を使う直接の O-C=N 型) は、橋頭
    //     原子の容量に依存しないため対象外のまま (これを誤って含めると
    //     ベンゾオキサゾロン等の正当な局所互変異性を壊す退行が生じた)。
    ...
    //     追加。97.61% → 97.94% (+25)。
    // I19 §3.4 追加修正 (2026-08-05): `seed_groups` の中心自身が価数
    //     スラック 0 (橋頭ヘテロ原子、全ての結合が恒久的に単結合) でも、
    //     その両隣の独立した実在二重結合を持つヘテロ原子 2 つを集約判定で
    //     誤って「受容体端点」とみなし、無関係な星型を形成する抜け穴が
    //     あった (例: `Oc1cc2cccnn2n1` の橋頭 N が両隣の N を拾い、環外
    //     フェノール性 OH の局所的な 2 端点群に誤って合流)。中心自身にも
    ...
    // I20 (2026-08-05): 標準 InChI の金属切断 + 多成分直列化を実装。
    //     標準 InChI は金属原子の全結合を切り、成分ごとに層を `;` で区切って
    //     並べる (`C[Hg]C` → `2CH3.Hg/h2*1H3;`)。切断時の電荷配分は相手で
    //     決まる: 金属-C/H は等方開裂 (電荷なし)、金属-ハロゲン等は異方開裂
    //     (配位子が陰イオン → 後段の中性化で `/p`、金属が陽イオン → `/q`)。
    //     成分順序は「炭素を含む成分が先 → 重原子数昇順 → H 数降順」
    //     (硫酸ナトリウムが `2Na.H2O4S` になる根拠)。同時に、それまで
    //     Unsupported で生成できなかった塩 (14 件) も出力対象になり、
    //     produced が 7439 → 7453 (全件) に。98.75% → 99.01% (+33)。
    // I21 (2026-08-05): メソ体の /m・/s 省略。実 InChI は構造と鏡像を両方
    //     正準化し、一致すれば (= アキラルなら) /m・/s を出さない。鏡像は
    //     全生パリティの反転なので、条件は「全パリティ反転を実現する
    //     グラフ自己同型 π が存在する」こと。π で番号を付け替えると中心 c の
    //     パリティは隣接の正準昇順が並べ替わる分だけ反転するので
    //     `old(c) XOR σ_c == !old(π(c))` を全中心で判定する。
    //     メソ酒石酸・cis-1,2-シクロヘキサンジオール等 3 件が一致し、
    //     光学活性な 47 件に回帰なし。99.01% → 99.05% (+3)。
    // I22 (2026-08-05): 可動 H 群の厳密検証フィルタ。現在の Kekule 構造の
    //     二重結合をマッチングとみなすと、供与体 d から受容体 a へ H が移動
    //     できる条件は「d を根とする M-交互パスで a に到達できる」ことと
    //     厳密に同値 (対称差の議論)。よって 1 通りの Kekule 構造だけで判定
    //     して過不足がなく、単純路制約は blossom の奇閉路収縮が正確に扱う。
    //     ただし「何を端点と認めるか」は実 InChI 固有の規則なので、端点認定は
    //     従来どおり seed_groups に任せ、この判定は検出済みの群から偽陽性を
    //     落とすフィルタとしてのみ使う (純粋 blossom で群を作ると 88.8% まで
    //     悪化することを実測で確認)。スルホニル S は二重結合を 2 本持つため
    //     容量 2 の頂点複製が必須 (単純マッチングだとスルホン酸の 3 つの O が
    //     1 群にならない)。99.05% → 99.37% (+24)。
    // I23 (2026-08-05): I19 §3.4 の poisoning 近似のうち `is_acceptor_from`
    //     での集約判定無効化を撤去。あれは「再 Kekule 化の連鎖が本当に成立
    //     するか」を局所情報で近似する回避策で、I22 の厳密な交互パス検証が
    //     その判定を正確に行えるようになったため不要かつ有害になっていた
    //     (橋頭 N を持つ縮環 `Oc1ccn2ccnc2n1` 型で本来より小さい群しか作れて
    //     いなかった 16 件が正しくなる)。ブリッジ探索の辺での poisoning は
    //     依然有効なので残す (外すと 99.40% に低下することを実測)。
    //     99.37% → 99.58% (+16)。
    // I24 (2026-08-05): 実 InChI (ichi_bns.c) と同じ **t-group ハブ**を実装。
    //     I22 の「供与体を根とする単一の M-交互パス」判定は H が 1 個だけ動く
    //     場合には厳密だが、ポリアザ縮環 (`[H]Oc1cc2c[nH]nc2nn1` 型) では
    //     2 個の可動 H が別々の群で同時に動いて初めて互変異性が成立し、
    //     対称差が 2 本の独立した交互パスに分かれるため永久に繋がらない。
    //     群ごとに可動 H 数だけの仮想ハブ頂点を置いて全端点と結ぶと、2 本が
    //     ハブ経由で 1 本の交互閉路になる。判定は最大マッチング (Edmonds、
    //     blossom.rs の max_matching) で「その端点の H 数が今と違う値も
    //     取れるか」を見る — 「受け取れるか」だけだと元々 H を持つ端点が
    //     自明に真になり固定 O-H を取り込み、「手放せるか」だけだと容量の
    //     都合で必ず H を持つアミジンの =NH を落とす。99.58% → 99.75% (+12)。
    // I25: ブリッジ探索の union に実 InChI のシフト位置条件 (taut_shift_ok) を
    //     課した。ブロッサム法の交互到達は「再 Kekule 化が成立するか」という
    //     必要条件しか見ないため、実 InChI が互変異性とみなさない 1,4 シフト
    //     (ニトロソ/アジドピロール、スルホン酸ピロール) や、交互環を経由しない
    ...
    //     99.83% → 99.97% (+11)。
    // I27: 同じ層最小化を /b 層にも広げた。無水物 `C/C=C/C(=O)OC(=O)/C=C\C` は
...
fn to_inchi_key_matches_rdkit_where_produced() {
    ...
    // I20 (2026-08-05): 金属切断 + 多成分直列化。98.80% → 99.06%。
    // I22 (2026-08-05): 可動 H 群の厳密検証フィルタ。99.06% → 99.39%。
